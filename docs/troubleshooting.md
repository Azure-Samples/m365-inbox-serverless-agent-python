...
- **`ModuleNotFoundError: agent_functions`**. The Functions host picked a Python worker that can't see the venv. `func5 run` (not bare `func5 start`) detects the uv-managed venv via `pyproject.toml` and selects the 3.13 worker. If you're on a shell where `func5` can't find the venv, `uv run func5 run` is the explicit fallback.
...
- Start Azurite yourself in a separate terminal before `func start`: `azurite --silent --skipApiVersionCheck --location .azurite`. v4 does not auto-launch it (v5 does, if `azurite` is on your `PATH`).
...
| Timer is not firing | Confirm the Functions host shows the timer trigger loaded at startup. `func5 run` launches Azurite automatically if it's on your `PATH`; pass `--no-azurite` only if you intentionally point `AzureWebJobsStorage` elsewhere. |
